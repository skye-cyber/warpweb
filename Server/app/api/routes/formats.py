...
from ...dependencies import get_format_service
...
async def get_format_info(
    ...
    if not info:
        raise HTTPException(
            status_code=404,
            detail=f"Format not found: {format_key}"
        )
...
async def suggest_output_format(
    ...
    if not suggested:
        ...
        if not info:
            raise HTTPException(
                status_code=404,
                detail=f"Format not found: {input_format}"
            )
...
async def get_conversion_matrix(
    ...
    for in_fmt in formats:
        row = {
            'input': in_fmt['key'],
            'can_convert_to': format_service.get_compatible_formats(in_fmt['key'], 'output')
        }
        matrix.append(row)
...
async def compare_formats(
    ...
    Compare two formats
    """
    info1 = format_service.get_format_info(format1) or format_service.get_format_by_extension(format1)
    info2 = format_service.get_format_info(format2) or format_service.get_format_by_extension(format2)
