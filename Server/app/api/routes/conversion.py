...
from ...models.responses import TaskResponse, OperationResult, ErrorResponse
...
from ...dependencies import get_progress_service, get_websocket_manager
...
async def submit_conversion(
    ...
    try:
        ...
        if operation_info and operation_info.get('requires_target_format', False):
            ...
            from_format = file_handler.get_file_info(request.input_paths[0])['extension']
            valid, message = conversion_service.validate_conversion(
                from_format,
                f".{request.target_format}"
            )
            if not valid:
...
async def validate_conversion(
    ...
    try:
        ...
        return {
            ...
            'input_files': [
                file_handler.get_file_info(p) for p in request.input_paths
            ]
        }
    ...
    for request in requests:
        try:
            ...
            tracker = progress_service.create_tracker(
                task_id,
                total_steps=10,
                description=f"Batch: {request.operation.value}"
            )
            tracker.start()
