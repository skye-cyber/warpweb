...
class FormatService:
    ...
    def _init_format_registry(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            'mp3': {
                ...
                'can_convert_from': ['.wav', '.ogg', '.flac', '.aac', '.mp4', '.avi', '.mkv']
            },
            'wav': {
                'name': 'WAV Audio',
                'category': 'audio',
                ...
                'can_convert_from': ['.mp3', '.ogg', '.flac', '.aac', '.mp4', '.avi', '.mkv']
            },
            'ogg': {
                'name': 'OGG Audio',
                'category': 'audio',
    ...
    def validate_conversion(self, input_format: str, output_format: str) -> Tuple[bool, str]:
        """Validate if conversion between formats is supported"""
        input_info = self.get_format_by_extension(input_format) or self.get_format_info(input_format)
        output_info = self.get_format_by_extension(output_format) or self.get_format_info(output_format)
        ...
        if output_ext not in input_info.get('can_convert_to', []):
            return False, f"Cannot convert from {input_info['name']} to {output_info['name']}"

        return True, "Conversion supported"

    def get_compatible_formats(self, format_key: str, direction: str = 'output') -> List[str]:
        """Get compatible formats for a given format"""
        info = self.get_format_info(format_key) or self.get_format_by_extension(format_key)
    ...
    def suggest_output_format(self, input_format: str, preferred: Optional[str] = None) -> Optional[str]:
        """Suggest an output format based on input"""
        info = self.get_format_by_extension(input_format) or self.get_format_info(input_format)
